...
const PageContainer: React.FC<any> = ({ children, enableLeftMargin }) => <div>{children}</div>;
...
export const Monitoring: React.FC = () => {
  ...
  const dataChartRef = useRef<DataChartFunction | null>(null);
  ...
  const processWebSocketMessage = useCallback(
        ...
        if (currentDataCount > maxBufferDataCount) {
          if (!isZoomedRefs.current[channelChartRef.current[channelID] || 'main']) {
            activePlotChannelsRef.current[channelID].dataPoints =
              activePlotChannelsRef.current[channelID].dataPoints.slice(-maxBufferDataCount);
          } else {
            const maxZoomedBuffDataCount = (1.0 / timeDelta) * 180;
            if (currentDataCount > maxZoomedBuffDataCount) {
              activePlotChannelsRef.current[channelID].dataPoints =
                activePlotChannelsRef.current[channelID].dataPoints.slice(-maxZoomedBuffDataCount);
            }
          }
        }
      }
    ...
    [bufferTimeWindow, activeDiscreteChannelGroup, isPlotPausedForAnalysis, activePlotChannelsRef, activeDiscreteChannelsRef, setTriggerChart, updateDataPoints]
  );
  ...
  }, [isStreaming, bufferTimeWindow, activeDiscreteChannelGroup, isPlotPausedForAnalysis, dataStreamWSRef, processWebSocketMessage]);
